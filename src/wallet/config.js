 ...
 * is active. The active network can be selected at startup via `SPLITKICK_NETWORK` *and* flipped
 ...
 * Overrides:  SPLITKICK_RPC (single RPC url), SPLITKICK_USDT (token address) — applied only to the
...
export const NETWORKS = {
  sepolia: {
    ...
    // company wallet via SPLITKICK_TREASURY in production.
  ...
  mainnet: {
    ...
    treasury: null, // real money — set SPLITKICK_TREASURY explicitly to enable the fee/pro path
    faucets: null // real money — no faucets
  }
}

/** The startup-default network key (default: sepolia). Env RPC/USDT overrides apply only here. */
export const DEFAULT_NETWORK = normalizeKey(env.SPLITKICK_NETWORK) || 'sepolia'
if (!NETWORKS[DEFAULT_NETWORK]) {
  throw new Error(`Unknown SPLITKICK_NETWORK="${env.SPLITKICK_NETWORK}". Use one of: ${Object.keys(NETWORKS).join(', ')}.`)
}
 ...
 * mainnet has no default — set SPLITKICK_TREASURY, otherwise the fee is disabled (`FEE.enabled`).
 * `SPLITKICK_TREASURY` env overrides the per-network default on every network.
 */
 ...
 * unless SPLITKICK_TREASURY is set.
 *   SPLITKICK_FEE_BPS  fee in basis points (default 50 = 0.50%)
 *   SPLITKICK_FEE_MIN  minimum fee per settle, in minor units / cents (default 0)
 *   SPLITKICK_FEE_MAX  cap on the fee per settle, in minor units / cents (default: uncapped)
 */
export const FEE = {
  bps: intEnv(env.SPLITKICK_FEE_BPS, 50),
  minMinor: intEnv(env.SPLITKICK_FEE_MIN, 0),
  maxMinor: env.SPLITKICK_FEE_MAX == null || env.SPLITKICK_FEE_MAX === '' ? null : intEnv(env.SPLITKICK_FEE_MAX, null),
 ...
 *   SPLITKICK_PRO_PRICE  price per month, in minor units (default 500 = 5.00 USD₮/mo)
 * Available only when a treasury address is configured (`enabled`).
 */
export const PRO = {
  pricePerMonthMinor: intEnv(env.SPLITKICK_PRO_PRICE, 500),
  get enabled () { return !!TREASURY.address }
}

/**
 * Switch the active network in place. Rewrites NETWORK/USDT/TREASURY and the faucet list so every
 * importer observes the change. Env SPLITKICK_RPC/SPLITKICK_USDT overrides are honored only for the
 * startup-default network (they're single-valued and would be wrong to carry onto the other chain).
 * SPLITKICK_TREASURY applies to every network.
 *
...
export function applyNetwork (key) {
  ...
  Object.assign(NETWORK, {
    ...
    chainId: sel.chainId,
    testnet: sel.testnet,
    rpcUrls: (isDefault && env.SPLITKICK_RPC) ? [env.SPLITKICK_RPC] : sel.rpcUrls,
    explorerTxUrl: sel.explorerTxUrl,
    explorerAddressUrl: sel.explorerAddressUrl
  })
  Object.assign(USDT, {
    address: (isDefault && env.SPLITKICK_USDT) ? env.SPLITKICK_USDT : sel.usdt.address,
    symbol: sel.usdt.symbol,
    decimals: sel.usdt.decimals
  })
  // SPLITKICK_TREASURY overrides on any network; otherwise the per-network default (null on mainnet).
  TREASURY.address = env.SPLITKICK_TREASURY || sel.treasury || null
