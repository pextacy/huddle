...
Dosyalar: `src/wallet/wdk.js`

- [ ] Seed phrase üret/geri yükle (`WDK.getRandomSeedPhrase(24)`); cihaz güvenli deposunda sakla.
      **Asla loglama, ledger'a yazma, replike etme** (claude.md secrets).
- [ ] WDK orchestrator + EVM modül kaydı (`registerWallet('ethereum', WalletManagerEvm, { provider })`).
- [ ] `getAddress()` ve `getBalance()` bir **testnet RPC**'ye karşı (FR-12).
- [ ] `sendUsdt(to, amountTokenUnits)` — gerçek ERC-20 USD₮ transferi; **tx hash** döndür (FR-10).
      Minor unit → 6 ondalıklı big-int dönüşümü **yalnızca bu sınırda** (claude.md money).
- [ ] Minimal cüzdan ekranı (solid renk): bakiye, adres + QR, ağ/zincir göstergesi (PRD §8 / FR-12).

**Done =** kendi-saklamalı cüzdandan testnet'te gerçek, on-chain USD₮ transferi gerçekleşir ve
tx hash görünür. (claude.md DoD: wallet/wdk)
